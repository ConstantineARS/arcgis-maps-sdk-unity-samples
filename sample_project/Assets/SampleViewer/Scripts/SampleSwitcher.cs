...
public class SampleSwitcher : MonoBehaviour
{
    ...
    {
        if (text == "WeatherQuery" && GraphicsSettings.renderPipelineAsset.ToString().Contains("Universal"))
        {
            nextSceneName = "WeatherQueryURP";
        }
        else
        {
            nextSceneName = text;
    ...
    {
        if (currentSceneName == "WeatherQuery")
        {
            if (PipelineType.ToString().Contains("HDRP"))
            {
                SceneManager.UnloadSceneAsync(currentSceneName);
                SceneManager.LoadSceneAsync("WeatherQueryURP", new LoadSceneParameters(LoadSceneMode.Additive));
                currentSceneName = "WeatherQueryURP";
                Debug.Log("URPSCENELOADED");

                // Testing potential lighting fixes

                string shaderPath = "Assets/SampleViewer/Samples/WeatherQuery/Sky/CloudShader.shadergraph";
            ...
            {
                SceneManager.UnloadSceneAsync(currentSceneName);
                SceneManager.LoadSceneAsync("WeatherQuery", new LoadSceneParameters(LoadSceneMode.Additive));
                currentSceneName = "WeatherQuery";
